...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (retrievedProfiles) { // Checking if the retrieved data is not null or undefined
        setProfileList(prevProfileList => ({...prevProfileList, ...retrievedProfiles}));
    }
}, []);
...
//Update Local Storage
  ...
  const handleRegister = (profile) => { //{ email: '',password: '',rePassword: '',first: '',last: '',nric: '',address: '',dob: '',contact: ''}
    setProfileList({...profileList, [profile.email]: {
  ...
  const handleProfileUpdate = (profile) => { //{ email:[currentEmail], profile: profileList[currentEmail] }
    setProfileList({...profileList, [profile.email]: profile.profile})
    // updateLocalStorage()
    setCurrentProfile(profile)
  }
  const handleSignInSuccess = (currentEmail) => {
    setCurrentProfile({ email:[currentEmail], profile: profileList[currentEmail] })
  }
